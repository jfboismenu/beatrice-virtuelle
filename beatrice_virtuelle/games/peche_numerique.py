...
def number_between():
    ...
    return (f"Quel est le nombre entre {answer - 1} et {answer + 1}", answer)


def number_before():
    answer = randint(10, 1000)
    return (f"Quel est le nombre avant {answer + 1}?", answer)


def number_after():
    answer = randint(10, 1000)
    return (f"Quel est le nombre après {answer - 1}?", answer)
...
def _scale_to_word(scale):
    return {1: "unité", 10: "dizaine", 100: "centaine"}[scale]
