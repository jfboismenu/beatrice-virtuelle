...
def new_game():
    return question(
        "Bienvenue dans la classe virtuelle de Béatrice. À quel jeu veux-tu jouer? "
        "Tu peux dire en tout temps Pêche Numérique ou Additions pour changer de jeu."
    )


games = {PecheNumerique.intent: PecheNumerique, Additions.intent: Additions}
...
def handle_answer(answer):
    ...
    if answer == "?":
        return question(
            " ".join(["Désolé, pourrais-tu répéter?", session.attributes["question"]])
        )

    expected_answer = session.attributes["answer"]

    print(answer, type(answer), expected_answer)
    result = handler.handle_answer(answer, expected_answer)
    next_question, next_answer = handler.ask_question()

    session.attributes["answer"] = next_answer
    return question(" ".join([result, "Prochaine question.", next_question]))
